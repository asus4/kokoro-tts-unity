...
namespace Kokoro
{
    ...
    public sealed class KokoroTTS : TextToSpeechInference
    {
        ...
        protected override int AudioSampleRate => SAMPLE_RATE;

        public KokoroTTS(byte[] modelData, Options options) : base(modelData, options)
        {
            inputs = new List<OrtValue>(3);

            inputTokensBuffer = ArrayPool<long>.Shared.Rent(512);
            voiceStyleBuffer = ArrayPool<float>.Shared.Rent(SAMPLE_RATE);
            outputAudioBuffer = ArrayPool<float>.Shared.Rent(STYLE_DIM);
        ...
        protected override ReadOnlyMemory<float> PostProcess(IReadOnlyList<OrtValue> outputs)
        {
            Assert.IsTrue(outputs.Count == 1, $"Expected 1 output, but got {outputs.Count}");

            var tmpAudioBuffer = outputs[0].GetTensorMutableDataAsSpan<float>();
            int length = tmpAudioBuffer.Length;

            EnsureBufferSize(ref outputAudioBuffer, length);
            var memory = new Memory<float>(outputAudioBuffer, 0, length);
            tmpAudioBuffer.CopyTo(memory.Span);

            return memory;
        }

        static void EnsureBufferSize<T>(ref T[] buffer, int length)
        {
